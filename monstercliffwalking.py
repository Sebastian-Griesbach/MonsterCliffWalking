...
class MonsterCliffWalkingEnv(Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        start_state_indices = np.array([self.szudzik_to_state[pair(self.player_start_state, monster_state)] for monster_state in range(self.nMonsterStates)])
    ...
    def _calculate_transition_prob(self, current_player_position, current_monster_position, delta):
        ...
        for potential_monster_position, probability in monster_transition_probabilities.items():
            ...
            if self._cliff[tuple(new_player_position)]:
                return_tuples.append((probability, self.szudzik_to_state[pair(self.player_start_state, new_monster_state)], -100, False))

            #player meets the monster
            elif tuple(new_player_position) == tuple(potential_monster_position):
                return_tuples.append((probability, self.szudzik_to_state[pair(new_player_state, new_monster_state)], -1000, True)) #Yes he dies

            #normal move
            else:
                terminal_state = (self.shape[0] - 1, self.shape[1] - 1)
                is_terminated = tuple(new_player_position) == terminal_state
                return_tuples.append((probability, self.szudzik_to_state[pair(new_player_state, new_monster_state)], -1, is_terminated))
